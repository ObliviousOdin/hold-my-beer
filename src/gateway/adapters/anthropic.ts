...
export const anthropicAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-anthropic-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Anthropic' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Anthropic' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Anthropic' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/v1/messages',
  notes: "Anthropic adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is x-api-key. Default base URL https://api.anthropic.com. Health probe /v1/messages. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://docs.anthropic.com.",
};

export const anthropicHeaderPlan = {
  auth: 'x-api-key',
  required: ['x-api-key'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const anthropicRetryPolicy = {
...
export function anthropicEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const anthropicRegionAffinity: Record<string, string[]> = {
...
export const anthropicErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Anthropic: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Anthropic: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Anthropic: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Anthropic: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Anthropic: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Anthropic: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Anthropic: Schema mismatch; check protocol of the selected target.',
};
