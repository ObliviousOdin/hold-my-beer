...
export const azureAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-azure-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Azure OpenAI' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Azure OpenAI' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Azure OpenAI' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Azure OpenAI adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is azure. Default base URL https://{resource}.openai.azure.com/openai. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://learn.microsoft.com/azure/ai-services.",
};

export const azureHeaderPlan = {
  auth: 'azure',
  required: ['api-key'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const azureRetryPolicy = {
...
export function azureEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const azureRegionAffinity: Record<string, string[]> = {
...
export const azureErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Azure OpenAI: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Azure OpenAI: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Azure OpenAI: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Azure OpenAI: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Azure OpenAI: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Azure OpenAI: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Azure OpenAI: Schema mismatch; check protocol of the selected target.',
};
