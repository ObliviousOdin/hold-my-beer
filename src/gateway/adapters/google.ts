...
export const googleAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-google-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Google' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Google' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Google' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Google adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://generativelanguage.googleapis.com/v1beta. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://ai.google.dev.",
};

export const googleHeaderPlan = {
  auth: 'bearer',
...
export function googleEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const googleRegionAffinity: Record<string, string[]> = {
...
export const googleErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Google: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Google: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Google: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Google: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Google: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Google: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Google: Schema mismatch; check protocol of the selected target.',
};
