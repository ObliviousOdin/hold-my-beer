...
export const openaiAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-openai-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'OpenAI' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'OpenAI' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'OpenAI' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "OpenAI adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.openai.com/v1. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://platform.openai.com/docs.",
};

export const openaiHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const openaiRetryPolicy = {
...
export function openaiEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const openaiRegionAffinity: Record<string, string[]> = {
...
export const openaiErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'OpenAI: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'OpenAI: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'OpenAI: Trim the prompt or pick a longer-context target.',
  'content_filter': 'OpenAI: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'OpenAI: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'OpenAI: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'OpenAI: Schema mismatch; check protocol of the selected target.',
};
