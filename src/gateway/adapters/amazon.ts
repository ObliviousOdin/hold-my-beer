...
export const amazonAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-amazon-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Amazon Bedrock' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Amazon Bedrock' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Amazon Bedrock' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/',
  notes: "Amazon Bedrock adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is aws. Default base URL https://bedrock-runtime.us-east-1.amazonaws.com. Health probe /. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://docs.aws.amazon.com/bedrock.",
};

export const amazonHeaderPlan = {
  auth: 'aws',
  required: ['api-key'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const amazonRetryPolicy = {
...
export function amazonEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const amazonRegionAffinity: Record<string, string[]> = {
...
export const amazonErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Amazon Bedrock: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Amazon Bedrock: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Amazon Bedrock: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Amazon Bedrock: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Amazon Bedrock: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Amazon Bedrock: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Amazon Bedrock: Schema mismatch; check protocol of the selected target.',
};
