...
export const deepinfraAdapter = {
  id: 'deepinfra' as ProviderId,
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-deepinfra-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'DeepInfra' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'DeepInfra' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'DeepInfra' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "DeepInfra adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.deepinfra.com/v1/openai. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://deepinfra.com/docs.",
};

export const deepinfraHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const deepinfraRetryPolicy = {
...
export function deepinfraEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const deepinfraRegionAffinity: Record<string, string[]> = {
...
export const deepinfraErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'DeepInfra: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'DeepInfra: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'DeepInfra: Trim the prompt or pick a longer-context target.',
  'content_filter': 'DeepInfra: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'DeepInfra: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'DeepInfra: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'DeepInfra: Schema mismatch; check protocol of the selected target.',
};
