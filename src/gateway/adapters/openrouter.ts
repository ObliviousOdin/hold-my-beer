...
export const openrouterAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-openrouter-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'OpenRouter' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'OpenRouter' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'OpenRouter' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "OpenRouter adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://openrouter.ai/api/v1. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://openrouter.ai/docs.",
};

export const openrouterHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const openrouterRetryPolicy = {
...
export function openrouterEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const openrouterRegionAffinity: Record<string, string[]> = {
...
export const openrouterErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'OpenRouter: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'OpenRouter: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'OpenRouter: Trim the prompt or pick a longer-context target.',
  'content_filter': 'OpenRouter: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'OpenRouter: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'OpenRouter: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'OpenRouter: Schema mismatch; check protocol of the selected target.',
};
