...
export const bedrockAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-bedrock-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Bedrock (direct)' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Bedrock (direct)' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Bedrock (direct)' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/',
  notes: "Bedrock (direct) adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is aws. Default base URL https://bedrock-runtime.us-west-2.amazonaws.com. Health probe /. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://docs.aws.amazon.com/bedrock.",
};

export const bedrockHeaderPlan = {
  auth: 'aws',
  required: ['api-key'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const bedrockRetryPolicy = {
...
export function bedrockEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const bedrockRegionAffinity: Record<string, string[]> = {
...
export const bedrockErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Bedrock (direct): Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Bedrock (direct): Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Bedrock (direct): Trim the prompt or pick a longer-context target.',
  'content_filter': 'Bedrock (direct): Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Bedrock (direct): Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Bedrock (direct): Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Bedrock (direct): Schema mismatch; check protocol of the selected target.',
};
