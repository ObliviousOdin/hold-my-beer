...
export const moonshotAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-moonshot-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Moonshot' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Moonshot' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Moonshot' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Moonshot adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.moonshot.ai/v1. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://platform.moonshot.ai.",
};

export const moonshotHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const moonshotRetryPolicy = {
...
export function moonshotEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const moonshotRegionAffinity: Record<string, string[]> = {
...
export const moonshotErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Moonshot: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Moonshot: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Moonshot: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Moonshot: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Moonshot: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Moonshot: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Moonshot: Schema mismatch; check protocol of the selected target.',
};
