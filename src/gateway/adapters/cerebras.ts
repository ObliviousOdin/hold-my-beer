...
export const cerebrasAdapter = {
  id: 'cerebras' as ProviderId,
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-cerebras-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Cerebras' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Cerebras' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Cerebras' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Cerebras adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.cerebras.ai/v1. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://inference-docs.cerebras.ai.",
};

export const cerebrasHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const cerebrasRetryPolicy = {
...
export function cerebrasEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const cerebrasRegionAffinity: Record<string, string[]> = {
...
export const cerebrasErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Cerebras: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Cerebras: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Cerebras: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Cerebras: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Cerebras: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Cerebras: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Cerebras: Schema mismatch; check protocol of the selected target.',
};
