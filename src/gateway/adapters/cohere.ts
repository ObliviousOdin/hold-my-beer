...
export const cohereAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-cohere-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Cohere' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Cohere' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Cohere' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Cohere adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.cohere.com/v2. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://docs.cohere.com.",
};

export const cohereHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const cohereRetryPolicy = {
...
export function cohereEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const cohereRegionAffinity: Record<string, string[]> = {
...
export const cohereErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Cohere: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Cohere: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Cohere: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Cohere: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Cohere: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Cohere: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Cohere: Schema mismatch; check protocol of the selected target.',
};
