...
export const vertexAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-vertex-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Vertex AI' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Vertex AI' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Vertex AI' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/',
  notes: "Vertex AI adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is gcp. Default base URL https://{region}-aiplatform.googleapis.com/v1. Health probe /. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://cloud.google.com/vertex-ai/docs.",
};

export const vertexHeaderPlan = {
  auth: 'gcp',
  required: ['api-key'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const vertexRetryPolicy = {
...
export function vertexEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const vertexRegionAffinity: Record<string, string[]> = {
...
export const vertexErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Vertex AI: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Vertex AI: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Vertex AI: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Vertex AI: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Vertex AI: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Vertex AI: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Vertex AI: Schema mismatch; check protocol of the selected target.',
};
