...
export const zaiAdapter = {
  ...
  fromUpstream(raw: Record<string, unknown>, req: CanonicalRequest, latencyMs: number): CanonicalResponse {
    ...
    return {
      id: String(raw.id ?? `hmb-zai-` + Math.random().toString(36).slice(2)),
      model: req.model,
      content,
      finish: "stop",
      usage,
  ...
  mapError(status: number, body: string): { status: number; code: string; message: string } {
    if (status === 401 || status === 403) return { status, code: "AUTH.UPSTREAM_DENIED", message: 'Z.AI' + " rejected the credential" };
    if (status === 429) return { status, code: "HLT.QUOTA", message: 'Z.AI' + " is rate limiting" };
    if (status >= 500) return { status, code: "HLT.UPSTREAM_5XX", message: 'Z.AI' + " is unhealthy: " + body.slice(0, 180) };
    return { status, code: "HLT.UPSTREAM", message: body.slice(0, 180) };
  },
  healthPath: '/models',
  notes: "Z.AI adapter maps canonical Hold My Beer requests onto the provider's native protocol. Auth style is bearer. Default base URL https://api.z.ai/api/paas/v4. Health probe /models. Streaming is framed as SSE for OpenAI-compatible hosts and as event-stream for Anthropic. The adapter never logs prompt bodies; only token counts, model ids, and latency land in evidence. Docs: https://docs.z.ai.",
};

export const zaiHeaderPlan = {
  auth: 'bearer',
  required: ['authorization'],
  redact: ['authorization', 'x-api-key', 'api-key'],
};

export const zaiRetryPolicy = {
...
export function zaiEstimate(inputTokens: number, outputTokens: number, inputRate: number, outputRate: number): number {
  return (inputTokens * inputRate + outputTokens * outputRate) / 1_000_000;
}

export const zaiRegionAffinity: Record<string, string[]> = {
...
export const zaiErrorAtlas: Record<string, string> = {
  'invalid_api_key': 'Z.AI: Rotate the BYOK credential; the gateway will fail closed.',
  'insufficient_quota': 'Z.AI: Provider quota is exhausted; route pool should failover.',
  'context_length_exceeded': 'Z.AI: Trim the prompt or pick a longer-context target.',
  'content_filter': 'Z.AI: Treat as POL.CONTENT and do not retry the same payload.',
  'overloaded': 'Z.AI: Open the circuit for cooldownMs and pick the next target.',
  'model_not_found': 'Z.AI: Alias points at a retired SKU; restore the previous route version.',
  'invalid_request': 'Z.AI: Schema mismatch; check protocol of the selected target.',
};
